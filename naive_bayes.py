...
def train_bayes_tree(model_save, features, labels):
...
if __name__ == '__main__':
    model_save = 'bayes.model'

    """ uncomment to train classifier """
    # init_movie2genres()
    # cal_popularity()
    # train_features, train_labels = get_training_set()
    # train_bayes_tree(model_save, train_features, train_labels)


    """ uncomment to predict """
    # with open(model_save, 'rb') as f:
    #     clf = pickle.load(f)
    # save_pred_res(clf, 'bayes_result.csv')



    """ tmp cross validation """
    model_tmp_save = 'tmp_bayes_crossval.model'
    init_movie2genres()
    cal_popularity()
    train_features, train_labels = get_training_set()
    X_train, X_test, y_train, y_test = train_test_split(
        train_features, train_labels, test_size=0.1, random_state=0)
    train_bayes_tree(model_tmp_save, train_features, train_labels)
